...
Each public image is a 1600-pixel square JPEG capped at 1 MiB (at most approximately 365 MiB/year of images). Copy/download produces PNG in the visitor's browser. Pending captures protect their referenced uploads from cleanup until the image is saved. Capture data and completed image references cannot be overwritten. The owner dashboard polls every 10 seconds while visible for today’s noon record and generates a PNG directly from its saved data. The archive also lets visitors open and export pending records immediately; it never substitutes the current board. This removes the GitHub runner queue from viewing/copying/downloading. Server JPEG processing remains asynchronous and does not guarantee a stored JPEG by 12:01. Browser export is bounded to 45 seconds, and failures retry in Admin. The Daily Snapshots tab pages through the archive; it does not post to social media.
